   ...
   function drawRadar($Object,$Values,$Format="")
    {
	 ...
	 $SetAxisLabelTextWithColor	= isset($Format["SetAxisLabelTextWithColor"]) ? $Format["SetAxisLabelTextWithColor"] : TRUE;
	 ...
	 $BackgroundSegmentColor = isset($Format["BackgroundSegmentColor"]) ? $Format["BackgroundSegmentColor"] : NULL;
     ...
     if ( $DrawBackground )
      {
       ...
       if ($BackgroundGradient == NULL)
       ...
       else
        {
         ...
         if ( $Layout == RADAR_LAYOUT_STAR )
          {
		   if($BackgroundSegmentColor == NULL) {
      ...
      if($BackgroundSegmentColor) {
		if ( $Layout == RADAR_LAYOUT_CIRCLE )
          {
           for($j=$Segments;$j>=1;$j--)
            {
			 $Color = array("R"=>255,"G"=>255,"B"=>255,"Alpha"=>100);
			 if($BackgroundSegmentColor[$j])
				$Color = array("R"=>$BackgroundSegmentColor[$j]["R"],"G"=>$BackgroundSegmentColor[$j]["G"],"B"=>$BackgroundSegmentColor[$j]["B"],"Alpha"=>$BackgroundSegmentColor[$j]["Alpha"]);
			 $Object->drawFilledCircle($CenterX,$CenterY,($EdgeHeight/$Segments)*$j,$Color);
     ...
     if ( $DrawAxisValues )
      {
       ...
       $Options["FontSize"] = $AxisFontSize;
		...
		// change this for axis label position
       //$Angle  = 360 / ($Points*2);
	   $Angle  = -90;
       for($j=1;$j<=$Segments;$j++)
        {
         $Label  = $j * $SegmentHeight;
		 if( $SetAxisLabelTextWithColor[$j]["Text"] ) {
			$Label  = $SetAxisLabelTextWithColor[$j]["Text"] ;
			$Options = array("R" => $SetAxisLabelTextWithColor[$j]["R"], "G" => $SetAxisLabelTextWithColor[$j]["G"], "B" => $SetAxisLabelTextWithColor[$j]["B"]);
         }
		 if ( $Layout == RADAR_LAYOUT_CIRCLE )
          {
           $EdgeX1 = cos(deg2rad($Angle+$AxisRotation)) * ($EdgeHeight/$Segments)*$j + $CenterX;
           $EdgeY1 = sin(deg2rad($Angle+$AxisRotation)) * ($EdgeHeight/$Segments)*$j + $CenterY + $EdgeHeight/($Segments*2);
          }
