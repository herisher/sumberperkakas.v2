 ...
 * sub category 2
 */
class Manager_SubCategory2Controller extends BaseController {
    const NAMESPACE_LIST = '/manager/sub-category2/list';
    
    /**
     * Search for creating conditions
     */
    private function createWherePhrase($order_by = 'disp_order asc') {
        $table = $this->model('Dao_SubCategory2');
        ...
        if ($session->post) {
            foreach ((array)$session->post as $key => $value) {
                ...
                if ( $key === 'sub_category1_id' && $value != NULL ) {
                    $where['sub_category1_id = ?'] = $value;
                }
    ...
    public function listAction() {
		...
		foreach ($this->view->paginator as $model) {
			$model = $model->toArray();
			if( $session->post['disp_flag'] && $session->post['category_id'] && $session->post['sub_category_id'] && $session->post['sub_category1_id'] && !$session->post['name']) {
				$model['up'] = 'Naik';
				$model['down'] = 'Turun';
			} else {
				$model['up'] = '';
			...
			$model['sub_category1'] = $this->model('Dao_SubCategory1')->retrieve($model['sub_category1_id']);
    ...
    private function doCreate($form, $datas) {
		...
		if( $position == 1 || !count($datas) ) { //first
			$sub_category2_id = $this->model('Logic_SubCategory2')->doNewTop($form->getValue('sub_category1_id'));
		} elseif( $position == 2 ) { //last
			$sub_category2_id = $this->model('Logic_SubCategory2')->doNewRow($form->getValue('sub_category1_id'));
		} elseif( $position == 3 ) { //after
			$sub_category2_id = $this->model('Logic_SubCategory2')->doInsert($form->getValue('sub_category1_id'), $form->getValue('disp_order'));
		}
    ...
    public function createAction() {
        ...
        $form->getElement('sub_category1_id')->setMultiOptions(array('' => '▼Pilih'));
        ...
        if ( $this->getRequest()->isPost() ) {
			...
			$form->getElement('disp_order')->setMultiOptions(array('' => '▼Pilih') + $sub_category2);
			$form->setDefault('disp_order', $this->getRequest()->getParam('disp_order'));
			
            if ( $this->createValid($form, $sub_category2) ) {
                $this->doCreate($form, $sub_category2);
            }
        }
    }
    ...
    public function editAction() {
        ...
        if ( $id && preg_match("/^\d+$/", $id) ) {
            $model = $this->model('Dao_SubCategory2')->retrieve($id);
    ...
    public function getDetailAction() {
        ...
        $q = $this->_getParam("q");
        $models = $this->model("Logic_SubCategory2")->findById($q);
		echo json_encode(
			array(
				"cid" => $models['category_id'],
				"scid" => $models['sub_category_id'],
				"sc1id" => $models['sub_category1_id']
			)
		);
	}
    ...
    public function getListAction() {
        $this->_helper->layout()->disableLayout();
        $q = $this->_getParam("q");
        $models = $this->model("Logic_SubCategory2")->getAllByCategory($q);
        $this->view->models = $models;
	}
    ...
    public function getListByCategoryAction() {
        $this->_helper->layout()->disableLayout();
        $q = $this->_getParam("q");
        $models = $this->model("Logic_SubCategory2")->getAllByMainCategory($q);
        $this->view->models = $models;
	}
	
    public function getListBySubcategoryAction() {
        $this->_helper->layout()->disableLayout();
        $q = $this->_getParam("q");
        $models = $this->model("Logic_SubCategory2")->getAllBySubCategory($q);
        $this->view->models = $models;
	}
	
	public function doupAction() {
        $id = $this->getRequest()->getParam('id');
        $sc1id = $this->getRequest()->getParam('sc1id');
		$this->model('Logic_SubCategory2')->doUp($sc1id, $id);
		$this->_redirect('/manager/sub-category2/list');
	}
	
	public function dodownAction() {
        $id = $this->getRequest()->getParam('id');
        $sc1id = $this->getRequest()->getParam('sc1id');
		$this->model('Logic_SubCategory2')->doDown($sc1id, $id);
		$this->_redirect('/manager/sub-category2/list');
	}
}
